
#include "pch.h"
#include "Bc7Core.h"
#include "Bc7Tables.h"

#include "SnippetDecompressIndexedSubset.h"
#include "SnippetInsertRemoveZeroBit.h"
#include "SnippetComputeOpaqueSubset2.h"
#include "SnippetLevelsMinimum.h"
#include "SnippetLevelsBuffer.h"

// https://docs.microsoft.com/en-us/windows/desktop/direct3d11/bc7-format-mode-reference#mode-2

namespace Mode2 {

#if defined(OPTION_COUNTERS)
	static std::atomic_int gComputeSubsetError2, gComputeSubsetError2GR, gComputeSubsetError2GB;
#endif

	void DecompressBlock(uint8_t input[16], Cell& output) noexcept
	{
		uint64_t data0 = *(const uint64_t*)&input[0];
		uint64_t data1 = *(const uint64_t*)&input[8];

		data0 >>= 3;

		size_t partitionIndex = data0 & 0x3F; data0 >>= 6;

		__m128i mc0 = _mm_setzero_si128();
		__m128i mc1 = _mm_setzero_si128();
		__m128i mc2 = _mm_setzero_si128();

		mc0 = _mm_insert_epi16(mc0, static_cast<int>(data0), 4); data0 >>= 5;
		mc0 = _mm_insert_epi16(mc0, static_cast<int>(data0), 5); data0 >>= 5;
		mc1 = _mm_insert_epi16(mc1, static_cast<int>(data0), 4); data0 >>= 5;
		mc1 = _mm_insert_epi16(mc1, static_cast<int>(data0), 5); data0 >>= 5;
		mc2 = _mm_insert_epi16(mc2, static_cast<int>(data0), 4); data0 >>= 5;
		mc2 = _mm_insert_epi16(mc2, static_cast<int>(data0), 5); data0 >>= 5;

		mc0 = _mm_insert_epi16(mc0, static_cast<int>(data0), 2); data0 >>= 5;
		mc0 = _mm_insert_epi16(mc0, static_cast<int>(data0), 3); data0 >>= 5;
		mc1 = _mm_insert_epi16(mc1, static_cast<int>(data0), 2); data0 >>= 5;
		mc1 = _mm_insert_epi16(mc1, static_cast<int>(data0), 3); data0 >>= 5;
		mc2 = _mm_insert_epi16(mc2, static_cast<int>(data0), 2);
		mc2 = _mm_insert_epi16(mc2, static_cast<int>(data1), 3); data1 >>= 5;

		mc0 = _mm_insert_epi16(mc0, static_cast<int>(data1), 6); data1 >>= 5;
		mc0 = _mm_insert_epi16(mc0, static_cast<int>(data1), 7); data1 >>= 5;
		mc1 = _mm_insert_epi16(mc1, static_cast<int>(data1), 6); data1 >>= 5;
		mc1 = _mm_insert_epi16(mc1, static_cast<int>(data1), 7); data1 >>= 5;
		mc2 = _mm_insert_epi16(mc2, static_cast<int>(data1), 6); data1 >>= 5;
		mc2 = _mm_insert_epi16(mc2, static_cast<int>(data1), 7); data1 >>= 5;

		const __m128i m5 = _mm_set1_epi16(0x1F);
		mc0 = _mm_and_si128(mc0, m5);
		mc1 = _mm_and_si128(mc1, m5);
		mc2 = _mm_and_si128(mc2, m5);

		mc0 = _mm_or_si128(_mm_slli_epi16(mc0, 3), _mm_srli_epi16(mc0, 2));
		mc1 = _mm_or_si128(_mm_slli_epi16(mc1, 3), _mm_srli_epi16(mc1, 2));
		mc2 = _mm_or_si128(_mm_slli_epi16(mc2, 3), _mm_srli_epi16(mc2, 2));

		const __m128i mopaque = _mm_set_epi16(0, 0, 0, 0, 0, 0, 255, 255);
		mc0 = _mm_or_si128(mc0, mopaque);
		mc1 = _mm_or_si128(mc1, mopaque);
		mc2 = _mm_or_si128(mc2, mopaque);

		data1 = InsertZeroBit(data1, gTableShrinkedLow3[partitionIndex] * 2);
		data1 = InsertZeroBit(data1, gTableShrinkedHigh3[partitionIndex] * 2);
		data1 = InsertZeroBit(data1, 1);

		DecompressIndexedSubset<2>(mc0, gTableSelection13[partitionIndex], (int*)output.ImageRows_U8, data1);
		DecompressIndexedSubset<2>(mc1, gTableSelection23[partitionIndex], (int*)output.ImageRows_U8, data1);
		DecompressIndexedSubset<2>(mc2, gTableSelection33[partitionIndex], (int*)output.ImageRows_U8, data1);

		output.BestParameter = partitionIndex;
		output.BestMode = 2;
	}

	static INLINED void ComposeBlock(uint8_t output[16], __m128i mc0, __m128i mc1, __m128i mc2, uint64_t indices, const size_t partitionIndex) noexcept
	{
		uint64_t data1 = RemoveZeroBit(RemoveZeroBit(RemoveZeroBit(indices,
			1),
			gTableShrinkedHigh3[partitionIndex] * 2),
			gTableShrinkedLow3[partitionIndex] * 2);

		mc0 = _mm_srli_epi16(mc0, 3);
		mc1 = _mm_srli_epi16(mc1, 3);
		mc2 = _mm_srli_epi16(mc2, 3);

		data1 <<= 5; data1 |= _mm_extract_epi16(mc2, 7);
		data1 <<= 5; data1 |= _mm_extract_epi16(mc2, 6);
		data1 <<= 5; data1 |= _mm_extract_epi16(mc1, 7);
		data1 <<= 5; data1 |= _mm_extract_epi16(mc1, 6);
		data1 <<= 5; data1 |= _mm_extract_epi16(mc0, 7);
		data1 <<= 5; data1 |= _mm_extract_epi16(mc0, 6);

		data1 <<= 5; data1 |= _mm_extract_epi16(mc2, 3);
		uint64_t data0 = _mm_extract_epi16(mc2, 2);
		data0 <<= 5; data0 |= _mm_extract_epi16(mc1, 3);
		data0 <<= 5; data0 |= _mm_extract_epi16(mc1, 2);
		data0 <<= 5; data0 |= _mm_extract_epi16(mc0, 3);
		data0 <<= 5; data0 |= _mm_extract_epi16(mc0, 2);

		data0 <<= 5; data0 |= _mm_extract_epi16(mc2, 5);
		data0 <<= 5; data0 |= _mm_extract_epi16(mc2, 4);
		data0 <<= 5; data0 |= _mm_extract_epi16(mc1, 5);
		data0 <<= 5; data0 |= _mm_extract_epi16(mc1, 4);
		data0 <<= 5; data0 |= _mm_extract_epi16(mc0, 5);
		data0 <<= 5; data0 |= _mm_extract_epi16(mc0, 4);

		data0 <<= 6; data0 |= partitionIndex;

		data0 <<= 3; data0 |= 1 << 2;

		*(uint64_t*)&output[0] = data0;
		*(uint64_t*)&output[8] = data1;
	}

	void FinalPackBlock(uint8_t output[16], Cell& input) noexcept
	{
		const size_t partitionIndex = input.BestParameter;

		Area& area1 = GetArea(input.Area13[partitionIndex], input.LazyArea13[partitionIndex], input, gTableSelection13[partitionIndex]);
		Area& area2 = GetArea(input.Area23[partitionIndex], input.LazyArea23[partitionIndex], input, gTableSelection23[partitionIndex]);
		Area& area3 = GetArea(input.Area33[partitionIndex], input.LazyArea33[partitionIndex], input, gTableSelection33[partitionIndex]);

		__m128i mc0 = input.BestColor0;
		__m128i mc1 = input.BestColor1;
		__m128i mc2 = input.BestColor2;

		uint64_t indices = 0;
		input.Error.Alpha = input.OpaqueAlphaError;
		input.Error.Total = ComputeOpaqueSubsetTable2(area1, mc0, indices);
		input.Error.Total += ComputeOpaqueSubsetTable2(area2, mc1, indices);
		input.Error.Total += ComputeOpaqueSubsetTable2(area3, mc2, indices);

		AreaReduceTable2(area1, mc0, indices);
		AreaReduceTable2(area2, mc1, indices);
		AreaReduceTable2(area3, mc2, indices);

		ComposeBlock(output, mc0, mc1, mc2, indices, partitionIndex);
	}

	static INLINED int CompressSubsetFast(const Area& area, __m128i& mc, int water) noexcept
	{
		mc = area.Bounds_U16;

		const __m128i mh5 = _mm_set1_epi16(0xF8);
		mc = _mm_and_si128(mc, mh5);

		mc = _mm_or_si128(mc, _mm_srli_epi16(mc, 5));

#if defined(OPTION_COUNTERS)
		gComputeSubsetError2++;
#endif
		return ComputeOpaqueSubsetError2(area, mc, _mm_cvtsi32_si128(water));
	}

	void CompressBlockFast(Cell& input) noexcept
	{
		for (size_t partitionIndex = 0; partitionIndex < 64; partitionIndex++)
		{
			__m128i mc0 = _mm_setzero_si128();
			__m128i mc1 = _mm_setzero_si128();
			__m128i mc2 = _mm_setzero_si128();

			int error = input.OpaqueAlphaError;
			if (error < input.Error.Total)
			{
				Area& area1 = GetArea(input.Area13[partitionIndex], input.LazyArea13[partitionIndex], input, gTableSelection13[partitionIndex]);

				error += CompressSubsetFast(area1, mc0, input.Error.Total - error);

				if (error < input.Error.Total)
				{
					Area& area2 = GetArea(input.Area23[partitionIndex], input.LazyArea23[partitionIndex], input, gTableSelection23[partitionIndex]);

					error += CompressSubsetFast(area2, mc1, input.Error.Total - error);

					if (error < input.Error.Total)
					{
						Area& area3 = GetArea(input.Area33[partitionIndex], input.LazyArea33[partitionIndex], input, gTableSelection33[partitionIndex]);

						error += CompressSubsetFast(area3, mc2, input.Error.Total - error);

						if (input.Error.Total > error)
						{
							input.Error.Total = error;

							input.BestColor0 = mc0;
							input.BestColor1 = mc1;
							input.BestColor2 = mc2;
							input.BestParameter = partitionIndex;
							input.BestMode = 2;

							if (error <= input.OpaqueAlphaError)
								return;
						}
					}
				}
			}
		}
	}

	struct Estimation
	{
		int ch1, ch2, ch3;
	};

	class Subset final
	{
	public:
		LevelsBuffer<48> ch1, ch2, ch3;

		INLINED Subset() noexcept = default;

		INLINED bool InitLevels(const Area& area, const int water, const Estimation& estimation) noexcept
		{
			ch1.ComputeChannelLevelsReduced<5, -1, false, gTableLevels2_Value5_U16>(area, 1, kGreen, water - estimation.ch2 - estimation.ch3);
			int min1 = ch1.MinErr;
			if (min1 >= water)
				return false;

			ch2.ComputeChannelLevelsReduced<5, -1, false, gTableLevels2_Value5_U16>(area, 2, kRed, water - min1 - estimation.ch3);
			int min2 = ch2.MinErr;
			if (min1 + min2 >= water)
				return false;

			ch3.ComputeChannelLevelsReduced<5, -1, false, gTableLevels2_Value5_U16>(area, 3, kBlue, water - min1 - min2);
			int min3 = ch3.MinErr;
			if (min1 + min2 + min3 >= water)
				return false;

			return true;
		}

		INLINED int TryVariants(const Area& area, __m128i& best_color, int water) noexcept
		{
			int min1 = ch1.MinErr;
			int min2 = ch2.MinErr;
			int min3 = ch3.MinErr;
			if (min1 + min2 + min3 >= water)
				return water;

			int n1 = ch1.Count;
			int n2 = ch2.Count;
			int n3 = ch3.Count;

			int memGB[48];

			for (int i1 = 0; i1 < n1; i1++)
			{
				int e1 = ch1.Err[i1].Error;
				if (e1 + min2 + min3 >= water)
					break;

				int c1 = ch1.Err[i1].Color;

				for (int i = 0; i < n3; i++)
				{
					memGB[i] = -1;
				}

				for (int i2 = 0; i2 < n2; i2++)
				{
					int e2 = ch2.Err[i2].Error + e1;
					if (e2 + min3 >= water)
						break;

					int c2 = ch2.Err[i2].Color;

					{
						__m128i mc = _mm_setzero_si128();
						mc = _mm_insert_epi16(mc, c1, 1);
						mc = _mm_insert_epi16(mc, c2, 2);
						mc = _mm_cvtepu8_epi16(mc);

#if defined(OPTION_COUNTERS)
						gComputeSubsetError2GR++;
#endif
						e2 = ComputeOpaqueSubsetError2Pair<_MM_SHUFFLE(2, 1, 2, 1)>(area, mc, gWeightsGRGR, gFixWeightsGR, _mm_cvtsi32_si128(water - min3));
						if (e2 + min3 >= water)
							continue;
					}

					for (int i3 = 0; i3 < n3; i3++)
					{
						int e3 = ch3.Err[i3].Error + e2;
						if (e3 >= water)
							break;

						int c3 = ch3.Err[i3].Color;

						int egb = memGB[i3];
						if (egb < 0)
						{
							__m128i mc = _mm_setzero_si128();
							mc = _mm_insert_epi16(mc, c1, 1);
							mc = _mm_insert_epi16(mc, c3, 3);
							mc = _mm_cvtepu8_epi16(mc);

#if defined(OPTION_COUNTERS)
							gComputeSubsetError2GB++;
#endif
							egb = ComputeOpaqueSubsetError2Pair<_MM_SHUFFLE(3, 1, 3, 1)>(area, mc, gWeightsGBGB, gFixWeightsGB, _mm_cvtsi32_si128(water - min2));
							memGB[i3] = egb;
						}
						if (egb + min2 >= water)
							continue;

						__m128i mc = _mm_setzero_si128();
						mc = _mm_insert_epi16(mc, c1, 1);
						mc = _mm_insert_epi16(mc, c2, 2);
						mc = _mm_insert_epi16(mc, c3, 3);
						mc = _mm_cvtepu8_epi16(mc);

#if defined(OPTION_COUNTERS)
						gComputeSubsetError2++;
#endif
						int err = ComputeOpaqueSubsetError2(area, mc, _mm_cvtsi32_si128(water));

						if (water > err)
						{
							water = err;

							best_color = mc;
						}
					}
				}
			}

			return water;
		}
	};

	void CompressBlock(Cell& input) noexcept
	{
		const size_t partitionIndex = input.BestParameter;

		__m128i mc0 = input.BestColor0;
		__m128i mc1 = input.BestColor1;
		__m128i mc2 = input.BestColor2;

		const Estimation estimation{ 0, 0, 0 };

		int error = input.OpaqueAlphaError;
		if (error < input.Error.Total)
		{
			Area& area1 = GetArea(input.Area13[partitionIndex], input.LazyArea13[partitionIndex], input, gTableSelection13[partitionIndex]);

#if defined(OPTION_COUNTERS)
			gComputeSubsetError2++;
#endif
			int water1 = ComputeOpaqueSubsetError2(area1, mc0, _mm_cvtsi32_si128(kBlockMaximalColorError));
			if (water1)
			{
				Subset subset1;
				if (subset1.InitLevels(area1, water1, estimation))
				{
					error += subset1.TryVariants(area1, mc0, water1);
				}
				else
				{
					error += water1;
				}
			}

			Area& area2 = GetArea(input.Area23[partitionIndex], input.LazyArea23[partitionIndex], input, gTableSelection23[partitionIndex]);

#if defined(OPTION_COUNTERS)
			gComputeSubsetError2++;
#endif
			int water2 = ComputeOpaqueSubsetError2(area2, mc1, _mm_cvtsi32_si128(kBlockMaximalColorError));
			if (water2)
			{
				Subset subset2;
				if (subset2.InitLevels(area2, water2, estimation))
				{
					error += subset2.TryVariants(area2, mc1, water2);
				}
				else
				{
					error += water2;
				}
			}

			Area& area3 = GetArea(input.Area33[partitionIndex], input.LazyArea33[partitionIndex], input, gTableSelection33[partitionIndex]);

#if defined(OPTION_COUNTERS)
			gComputeSubsetError2++;
#endif
			int water3 = ComputeOpaqueSubsetError2(area3, mc2, _mm_cvtsi32_si128(kBlockMaximalColorError));
			if (water3)
			{
				Subset subset3;
				if (subset3.InitLevels(area3, water3, estimation))
				{
					error += subset3.TryVariants(area3, mc2, water3);
				}
				else
				{
					error += water3;
				}
			}

			if (input.Error.Total > error)
			{
				input.Error.Total = error;

				input.BestColor0 = mc0;
				input.BestColor1 = mc1;
				input.BestColor2 = mc2;
				//input.BestParameter = partitionIndex;
				//input.BestMode = 2;
			}
		}
	}

	static INLINED int EstimateLevels(const Area& area, const int water, Estimation& estimation) noexcept
	{
		int error = 0;
		if (error < water)
		{
			int level1 = LevelsMinimum::EstimateChannelLevelsReduced<5, false, gTableLevels2_Value5_U16, gTableCutLevels2_Value5_U16>(area, 1, kGreen, water - error);
			estimation.ch1 = level1;
			error += level1;

			if (error < water)
			{
				int level2 = LevelsMinimum::EstimateChannelLevelsReduced<5, false, gTableLevels2_Value5_U16, gTableCutLevels2_Value5_U16>(area, 2, kRed, water - error);
				estimation.ch2 = level2;
				error += level2;

				if (error < water)
				{
					int level3 = LevelsMinimum::EstimateChannelLevelsReduced<5, false, gTableLevels2_Value5_U16, gTableCutLevels2_Value5_U16>(area, 3, kBlue, water - error);
					estimation.ch3 = level3;
					error += level3;

					if (error < water)
					{
						return error;
					}
				}
			}
		}

		return water;
	}

	void CompressBlockFull(Cell& input) noexcept
	{
		Node order[64];
		Estimation estimations1[64];
		Estimation estimations2[64];
		Estimation estimations3[64];
		int lines2[64];
		int lines3[64];

		size_t partitionsCount = 0;
		for (size_t partitionIndex = 0; partitionIndex < 64; partitionIndex++)
		{
			if ((input.PersonalMode == 2) && (input.PersonalParameter == partitionIndex))
				continue;

			Area& area1 = GetArea(input.Area13[partitionIndex], input.LazyArea13[partitionIndex], input, gTableSelection13[partitionIndex]);

			const int water1 = input.Error.Total - input.OpaqueAlphaError;
			int line1 = EstimateLevels(area1, water1, estimations1[partitionIndex]);
			if (line1 < water1)
			{
				Area& area2 = GetArea(input.Area23[partitionIndex], input.LazyArea23[partitionIndex], input, gTableSelection23[partitionIndex]);

				const int water2 = water1 - line1;
				int line2 = EstimateLevels(area2, water2, estimations2[partitionIndex]);
				if (line2 < water2)
				{
					Area& area3 = GetArea(input.Area33[partitionIndex], input.LazyArea33[partitionIndex], input, gTableSelection33[partitionIndex]);

					const int water3 = water2 - line2;
					int line3 = EstimateLevels(area3, water3, estimations3[partitionIndex]);
					if (line3 < water3)
					{
						lines2[partitionIndex] = line2;
						lines3[partitionIndex] = line3;

						order[partitionsCount++].Init(input.OpaqueAlphaError + line1 + line2 + line3, static_cast<int>(partitionIndex));
					}
				}
			}
		}

		{
			Node order2[64];
			if (radix_sort(order, order2, partitionsCount) == order2)
			{
				memcpy(order, order2, sizeof(Node) * partitionsCount);
			}
		}

		for (size_t i = 0; i < partitionsCount; i++)
		{
			if (order[i].Error < input.Error.Total)
			{
				const size_t partitionIndex = order[i].Color;

				__m128i mc0 = _mm_setzero_si128();
				__m128i mc1 = _mm_setzero_si128();
				__m128i mc2 = _mm_setzero_si128();

				const int line3 = lines3[partitionIndex];

				Area& area1 = GetArea(input.Area13[partitionIndex], input.LazyArea13[partitionIndex], input, gTableSelection13[partitionIndex]);

				const int water1 = input.Error.Total - input.OpaqueAlphaError - lines2[partitionIndex] - line3;
				Subset subset1;
				if (subset1.InitLevels(area1, water1, estimations1[partitionIndex]))
				{
					int error = subset1.TryVariants(area1, mc0, water1);
					if (error < water1)
					{
						error += input.OpaqueAlphaError;

						Area& area2 = GetArea(input.Area23[partitionIndex], input.LazyArea23[partitionIndex], input, gTableSelection23[partitionIndex]);

						const int water2 = input.Error.Total - error - line3;
						Subset subset2;
						if (subset2.InitLevels(area2, water2, estimations2[partitionIndex]))
						{
							error += subset2.TryVariants(area2, mc1, water2);

							if (error < input.Error.Total - line3)
							{
								Area& area3 = GetArea(input.Area33[partitionIndex], input.LazyArea33[partitionIndex], input, gTableSelection33[partitionIndex]);

								const int water3 = input.Error.Total - error;
								Subset subset3;
								if (subset3.InitLevels(area3, water3, estimations3[partitionIndex]))
								{
									error += subset3.TryVariants(area3, mc2, water3);

									if (input.Error.Total > error)
									{
										input.Error.Total = error;

										input.BestColor0 = mc0;
										input.BestColor1 = mc1;
										input.BestColor2 = mc2;
										input.BestParameter = partitionIndex;
										input.BestMode = 2;

										if (error <= input.OpaqueAlphaError)
											return;
									}
								}
							}
						}
					}
				}
			}
		}
	}

	void PrintCounters() noexcept
	{
#if defined(OPTION_COUNTERS)
		PRINTF("[Mode 2]\tGR2 = %i, GB2 = %i, GRB2 = %i",
			gComputeSubsetError2GR.load(), gComputeSubsetError2GB.load(), gComputeSubsetError2.load());
#endif
	}

} // namespace Mode2
