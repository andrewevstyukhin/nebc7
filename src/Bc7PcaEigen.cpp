...
int PrincipalComponentAnalysis3(const Area& area)
{
	...
	const __m128 mu = _mm_set_ps(V(2, 0), V(1, 0), V(0, 0), 0.f);
	...
	const __m128i mweights = _mm_set_epi32(kBlue, kRed, kGreen, 0);
...
int PrincipalComponentAnalysis4(const Area& area)
{
	...
	Eigen::JacobiSVD<A, Eigen::HouseholderQRPreconditioner> svd(matrix, Eigen::ComputeThinV);
	const auto& V = svd.matrixV();
	const __m128 mu = _mm_set_ps(V(3, 0), V(2, 0), V(1, 0), V(0, 0));
	...
	for (int j = 0; j < R; j++)
	{
		__m128 mx = _mm_load_ps(&matrix(j, 0));
